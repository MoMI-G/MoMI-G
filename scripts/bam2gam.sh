...
# ex. bash vcf2xg.sh test.bam test.xg /bin/vg
...
cat $bed_file.1 | ruby bam2gam/uuid.rb | awk '$1!~/_/{print $0}' | grep -v "chrM" | grep -v "chrEBV" > $bed_file.2
...
cat $gam_json_file | ruby bam2gam/json.rb > $gam_json_file.2
